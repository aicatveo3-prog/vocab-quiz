/**
 * words-f.js 검사 — 매 차수마다 돌린다.
 *
 *   node tools/words-f-audit.js           전체 검사
 *   node tools/words-f-audit.js --rules   규칙별 검사 건수 (죽은 검사 탐지)
 *
 * 검사 6종
...
['js/data/words.js', 'js/data/words-b.js', 'js/data/words-c.js', 'js/data/words-d.js',
 'js/data/words-e.js', 'js/data/words-f.js', 'js/data/words-g.js', 'js/data/words-h.js', 'js/data/words-i.js', 'js/data/words-j.js', 'js/data/words-k.js', 'js/data/words-l.js', 'js/data/words-m.js', 'js/data/words-n.js', 'js/data/words-o.js', 'js/data/words-p.js', 'js/data/gloss.js', 'js/data/pron.js',
 'js/quizgen.js'].forEach(load);

var F = window.VOCAB_F || [];
...
F.forEach(function (x) {
  ...
  if (seen[l]) errors.push(at + ' F 세트 안에서 표제어 중복');
  seen[l] = 1;
...
function tally(key) {
  var t = {};
  F.forEach(function (x) { t[x[key]] = (t[x[key]] || 0) + 1; });
  return Object.keys(t).sort().map(function (k) { return k + ' ' + t[k]; }).join('  ');
}

/* ── 출력 ─────────────────────────────────── */
console.log('── words-f.js 검사 ─────────────────────────');
console.log('표제어 : ' + F.length + '개');
console.log('품사   : ' + tally('pos'));
console.log('레벨   : ' + tally('level'));
console.log('');
console.log('출제 시뮬레이션 (quizgen 실제 빌더)');
console.log('  4지선다    ' + sim.mcq + ' / ' + F.length);
console.log('  아닌것     ' + sim.not + ' / ' + F.filter(function (x) { return x.syn && x.syn.length >= 3; }).length);
console.log('  문장빈칸   ' + sim.cloze + ' / ' + F.filter(function (x) { return x.ex && x.ex.length; }).length);
console.log('  짝맞추기   ' + sim.match + ' / ' + F.length);
console.log('');
console.log('syn 을 비워 둔 단어 : ' + F.filter(function (x) { return !x.syn || !x.syn.length; })
