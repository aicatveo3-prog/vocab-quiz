/**
 * words-g.js 검사 — 매 차수마다 돌린다.
 *
 *   node tools/words-g-audit.js           전체 검사
 *   node tools/words-g-audit.js --rules   규칙별 검사 건수 (죽은 검사 탐지)
 *
 * 검사 6종
...
G.forEach(function (x) {
  ...
  if (seen[l]) errors.push(at + ' G 세트 안에서 표제어 중복');
  seen[l] = 1;
...
function tally(key) {
  var t = {};
  G.forEach(function (x) { t[x[key]] = (t[x[key]] || 0) + 1; });
  return Object.keys(t).sort().map(function (k) { return k + ' ' + t[k]; }).join('  ');
}

/* ── 출력 ─────────────────────────────────── */
console.log('── words-g.js 검사 ─────────────────────────');
console.log('표제어 : ' + G.length + '개');
console.log('품사   : ' + tally('pos'));
console.log('레벨   : ' + tally('level'));
console.log('');
console.log('출제 시뮬레이션 (quizgen 실제 빌더)');
console.log('  4지선다    ' + sim.mcq + ' / ' + G.length);
console.log('  아닌것     ' + sim.not + ' / ' + G.filter(function (x) { return x.syn && x.syn.length >= 3; }).length);
console.log('  문장빈칸   ' + sim.cloze + ' / ' + G.filter(function (x) { return x.ex && x.ex.length; }).length);
console.log('  짝맞추기   ' + sim.match + ' / ' + G.length);
console.log('');
console.log('syn 을 비워 둔 단어 : ' + G.filter(function (x) { return !x.syn || !x.syn.length; })
